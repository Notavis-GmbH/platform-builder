...
# Uninstall the CSI pipeline format service and script if installed.

set -euo pipefail

SERVICE_NAME="csi-pipeline-format.service"
SERVICE_PATH="/etc/systemd/system/${SERVICE_NAME}"
SCRIPT_DEST="/usr/local/sbin/set-csi-pipeline-format.sh"

echo "Uninstalling CSI pipeline format service..."
